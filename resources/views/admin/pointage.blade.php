...
<div class="col-sm-6">
        <h1 > <small><em>liste du pointages </em></small></h1>
      </div>  </br>  
      @if (session('success'))
...
@foreach ($pointages as $pointer)
          <tr>
              <td> {{$pointer->id}} </td>
              <td> {{$pointer->user->Fullname}}</td>
              <td> {{$pointer->user->email}}</td>

              <td> {{$pointer->jour}} </td>
              <td> {{$pointer->date}} </td>

              <td>  <a class="btn btn-danger" href="{{route('supprimerpointage',['id'=>$pointer->id])}}" role="button">Supprimer</a></td>
             
   
       
          </tr>
@endforeach
<a class="btn btn-primary" href="{{route('convocationAdmin')}}" role="button">Cliquez Ici pour envoyer une convovation</a>
            </table>     
       </div>         
